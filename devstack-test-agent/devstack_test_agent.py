...
Examples:
  ...
  # Post only the summary to the forge (not the full report)
  %(prog)s --change 982567 --post-summary
        """,
    )
    ...
    _summary_prompt = Path(__file__).parent / "prompts" / "devstack_test_summary_prompt.txt"
    ...
    if args.post_only:
        ...
        if not args.post_summary:
            ...
            review_info = parse_review_file(report)
            if review_info and needs_summary(args, config):
                summary = generate_summary(report, _summary_prompt, config)
                if summary:
                    forge = create_forge_client(config)
                    change_info = forge.get_change(review_info.change_number)
                    model_name = config.get("model", "claude-sonnet-4-6")
                    comment = (
                        f"*DevStack test summary by {model_name}*\n\n{summary}\n\n"
                        "---\n*This summary was generated by an AI and may contain errors.*"
                    )
                    forge.post_feedback(change_info, comment, vote=None, line_comments=[])
        return
    ...
    if needs_summary(args, config):
        ...
        if change_ref:
            change_id = change_ref.split("/")[-1].split("+")[-1].strip() \
                if change_ref.startswith("http") else change_ref
            ps_glob = f"ps{patchset}_" if patchset else "ps*_"
            pattern = f"testing_report_*_{change_id}_{ps_glob}*.md"
        else:
            pattern = "testing_report_*.md"
        report = find_latest_report(reviews_dir, pattern)
        summary = generate_summary(report, _summary_prompt, config) if report else None
        if summary:
            print_summary(summary, report)
            if args.post_summary and report:
                review_info = parse_review_file(report)
                if review_info:
                    forge = create_forge_client(config)
                    change_info = forge.get_change(review_info.change_number)
