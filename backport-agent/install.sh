...
usage() {
    ...
    echo ""
    echo "Install the Backport Agent into the Claude Agents virtual environment."
    echo ""
    echo "Options:"
...
echo -e "${GREEN}✓${NC} backport-agent installed (octavia-backport-monitor, octavia-backport-review)"
...
if [ -z "$INSTALL_SYSTEMD" ]; then
    echo -n "Install systemd service files for the backport agent? [y/N] "
    read -r _response
...
if [ "$INSTALL_SYSTEMD" = "yes" ]; then
    ...
    echo ""
    echo "To enable the backport monitor (daily at 08:00):"
    echo "  systemctl --user enable --now octavia-backport-monitor.timer"
    echo ""
    echo "To enable the backport review agent (every 4h):"
    echo "  systemctl --user enable --now octavia-backport-review.timer"
fi
