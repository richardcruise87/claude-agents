...
class TestBuildFeedbackComment:
    _REPORT = """\
## Bug Analysis

This is a valid bug affecting load balancer creation.

## Fix Strategy

Apply the patch to octavia/api/v2/controllers/load_balancer.py.
...
"""
    ...
    def test_strips_token_usage_section(self):
        result = build_feedback_comment(self._REPORT, "claude-sonnet-4-6")
        assert "Token Usage" not in result
        assert "5000" not in result

    def test_includes_attribution_header(self):
        result = build_feedback_comment(self._REPORT, "claude-sonnet-4-6")
        assert "*Generated by claude-sonnet-4-6*" in result

    def test_includes_disclaimer_footer(self):
        result = build_feedback_comment(self._REPORT, "claude-sonnet-4-6")
        assert "generated by an AI" in result

    def test_includes_report_body(self):
        result = build_feedback_comment(self._REPORT, "claude-sonnet-4-6")
        assert "Bug Analysis" in result
        assert "valid bug" in result

    def test_sanitizes_credentials(self):
        report = "## Analysis\n\nexport OS_PASSWORD=secret123\n\nGood bug.\n"
        result = build_feedback_comment(report, "model")
        assert "secret123" not in result
        assert "[REDACTED]" in result

    def test_caps_length(self):
        long_report = "## Analysis\n\n" + ("x" * 10_000)
        result = build_feedback_comment(long_report, "model", max_chars=500)
        # Body should be capped; attribution header/footer are extra
