...
def build_feedback_comment(report_content: str, model_name: str, max_chars: int = 6000) -> str:
    """Build a sanitized comment for posting back to an issue tracker.

    Strips the token-usage section appended by the agent runner, caps the
    body at ``max_chars``, sanitizes credentials, then wraps with an AI
    attribution header and disclaimer footer.

    Args:
        report_content: Full text of the generated report markdown.
        model_name:     Model identifier shown in the attribution line.
        max_chars:      Maximum body length before attribution/footer.

    Returns:
        Ready-to-post comment string.
    """
    # Strip token usage section (appended by runner, not part of the report)
    clean = re.split(r'\n---\n\n## Token Usage', report_content, maxsplit=1)[0].strip()

    if len(clean) > max_chars:
        cut = clean.rfind("\n\n", 0, max_chars)
        clean = clean[:cut] if cut > max_chars // 2 else clean[:max_chars]

    clean = sanitize_for_forge(clean)

    header = f"*Generated by {model_name}*\n\n"
...
def find_latest_report(
    ...
    exclude_suffix: if set, files whose stem ends with this string are skipped
    (e.g. "_context" to skip fix-proposal context packets).
    """
